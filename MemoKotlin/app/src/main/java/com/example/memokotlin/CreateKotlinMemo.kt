...
class CreateKotlinMemo : AppCompatActivity() {
    ...
    internal var helper: KtOpenHelper? = null
    // 新規フラグ
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // データベースから値を取得する
        if (helper == null) {
            helper = KtOpenHelper(this@CreateKotlinMemo)
        }
        ...
        registerButton.setOnClickListener {
            ...
            try {
                if (newFlag) {
                    ...
                    // INSERT
                    db.execSQL("insert into KT_MEMO_TABLE(uuid, body) VALUES('$id', '$bodyStr')")
                } else {
                    // UPDATE
                    db.execSQL("update KT_MEMO_TABLE set body = '$bodyStr' where uuid = '$id'")
                }
            } finally {
                // finallyは、tryの中で例外が発生した時でも必ず実行される
                // dbを開いたら確実にclose
